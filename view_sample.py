...
TYPES   = ['vis047','vis086','ir133','dsr','sza']
...
def parse_args():
    ...
    parser.add_argument("--catalog","-c",help='tile catalog',default='tiles.csv')
    parser.add_argument("--data_path","-d",help='Path to SolarSat data',default='data/')
    parser.add_argument("--id","-i",help='tile id (if not provided, a random case is selected',default=None)
...
def main():  
    """
    Function to create sample images of a single event in SolarSat
    ...
    USAGE:
        ...
        python view_sample.py    --output_location  OUTLOC  --catalog  tile.csv  --data_path  SOLARSAT_DATA_PATH  
    ...
    if args.id:
        tile_id = args.id
        cat_groups.get_group(tile_id) # make sure it's valid
    else:
        # pick random group with full sensor coverage 
        allsensors = (cat_groups.size()==5)
        ids = np.array(list(cat_groups.groups.keys()))
        tile_id = np.random.choice( ids[allsensors], 1 )[0]
        print('Using TILE ID',tile_id)
    output_location=f'{output_location}/{tile_id}'
    try:
    ...
    #print('Loading data')
    data = get_data(tile_id, cat_groups, args.data_path)

    #print('Making plots')
    make_images2(data, output_location, tile_id, res=args.resolution)
    update_progress(1.0)

def get_data( tile_id, grouped_catalog, path ):
    """ 
    returns dict { img_type : {"meta" : META, "data": DATA} }
    """
    cases = grouped_catalog.get_group(tile_id)
    data = {}
    for typ in TYPES:
        data[typ]={}
...
def make_insitu_plots(data,bmap,t0,t1,lght_times=None):
...
def get_title(typ):
    if typ=='dsr':
        ...
        return "GOES-16 VIS 047"
    elif typ=='vis086':
        return 'GOES-16 VIS 086'
    elif typ=='ir133':
        return 'GOES-16 IR 133'
    return ''
