...
class SOLARSAT(data.Dataset):
    ...
    def get_batch(self,idx, return_meta=False):
        ...
        # X = [data[t].astype(self.output_type) for t in self.x_img_types]
        X = np.array([data[t][0, :self.input_len, :, :].astype(np.float32) if data[t].ndim == 4 else data[t][:, :self.input_len] for t in self.x_img_types])
...
class SolarSatLightningDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self):
        data = SOLARSAT(tile_list=self.train_tile_list,year_list=self.train_year_list, input_len=self.input_len, output_len=self.output_len, normalize_max=self.normalize_max, downscale=self.downscale, train=True)
        return data
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "test" or stage is None:
            self.lstm_test = SOLARSAT(tile_list=self.test_tile_list,year_list=self.test_year_list, input_len=self.input_len, output_len=self.output_len, normalize_max=self.normalize_max, downscale=self.downscale, train=True)

        if stage == "predict" or stage is None:
            self.lstm_predict = SOLARSAT(tile_list=self.test_tile_list,year_list=self.test_year_list, input_len=self.input_len, output_len=self.output_len, normalize_max=self.normalize_max, downscale=self.downscale, train=True)
