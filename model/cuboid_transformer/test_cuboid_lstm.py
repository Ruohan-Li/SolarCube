...
def main():
    ...
    if args.cfg is not None:
        ...
        name_prefix=dataset_oc['dataset_name'][:-4]+'_'+str(dataset_oc['in_len'])+'_'+str(dataset_oc['out_len'])
        print(name_prefix)
    else:
    ...
    if args.test:
        ...
        checkpoint_state=pl_load(os.path.join(pl_module.save_dir, "checkpoints", ckpt_path), map_location=torch.device("cpu"))
        ...
        state_dict = pl_load(trainer.checkpoint_callback.best_model_path, map_location=torch.device("cpu"))
        torch.save(state_dict, os.path.join(pl_module.save_dir, "checkpoints", name_prefix+'.ckpt'))
