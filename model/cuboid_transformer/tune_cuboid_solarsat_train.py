...
def get_parser():
    ...
    parser.add_argument('--gpui', default=1, type=int)    #for specific gpu device, but no parallel
...
def main():
    ...
    # )
    # trainer_kwargs = pl_module.set_trainer_kwargs(
    #     devices=1,  # Explicitly setting to use one device, the CPU
    ...
    if args.test:
        ...
        np.savez_compressed(os.path.join('testing',oc_from_file.logging.version, name_prefix+'_transformer_prediction'), predictions)
        # else:
