...
from earthformer2.config import cfg
from earthformer2.utils.optim import SequentialLR, warmup_lambda
from earthformer2.utils.utils import get_parameter_names
from earthformer2.utils.checkpoint import pl_ckpt_to_pytorch_state_dict, s3_download_pretrained_ckpt
from earthformer2.utils.layout import layout_to_in_out_slice
from earthformer2.visualization.nbody import save_example_vis_results
#from earthformer2.metrics.sevir import SEVIRSkillScore
from earthformer2.cuboid_transformer.cuboid_transformer import CuboidTransformerModel
from earthformer2.datasets.LSTM.LSTM_torchlightning_wrap import LSTMLightningDataModule
from earthformer2.utils.apex_ddp import ApexDDPStrategy
...
def main():
    ...
    if args.test:
        ...
        predictions=trainer.predict(ckpt_path="best",
                     datamodule=dm)
        np.savez_compressed(os.path.join(pl_module.save_dir, "checkpoints/prediction/tune", name_prefix+'_train_prediction'), predictions)
        end=time.time()
        print(end-start)
