...
def main():
    ...
    if args.ckpt_name is not None:
        ...
        ckpt_path = None
    sample_loader=dm.test_dataloader()
    #print(sample_loader.shape())
    sample_batch1,sample_batch2 = next(iter(sample_loader))
    print(sample_batch2.shape)

    sample_batch1 = sample_batch1.to(pl_module.device)
    sample_batch2 = sample_batch2.to(pl_module.device)

    # Calculate FLOPs and parameters
    flops, params = profile(pl_module, inputs=([sample_batch1,sample_batch2],))
    print(f"Estimated FLOPs: {flops / 10**9:.2f} GFLOPs")
    print(f"Number of parameters: {params / 10**6:.2f} M")
